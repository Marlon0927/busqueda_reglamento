...
terminos_buscados = ["programa", "créditos"]

N = len(documents)

# Calcular IDF para cada término
idf_dict = {}
for termino in terminos_buscados:
    df = sum(1 for doc in documents if termino.lower() in doc.lower())
    idf_dict[termino] = np.log(N / df) if df > 0 else 0

print("IDF por término:")
for termino, idf in idf_dict.items():
    print(f"{termino}: {idf:.4f}")

# Calcular TF-IDF total por documento
scores = []
...
for i, doc in enumerate(documents):
    ...
    for termino in terminos_buscados:
        tf = palabras.count(termino.lower()) / len(palabras)
        tf_idf = tf * idf_dict[termino]
        total_score += tf_idf

    scores.append((i + 1, total_score, doc))

# Ordenar resultados
scores.sort(key=lambda x: x[1], reverse=True)

# Mostrar resultados
for pos, (doc_id, score, doc) in enumerate(scores, 1):
    print(f"{pos} - Documento {doc_id}: Score = {score:.4f}")
    print(doc)
    print()